const REQUIRED = 'Você deve preencher todos os campos antes de continuar';

const PHOTO = 'Uma foto deve ser informada para continuar';

const FULLNAME = 'O nome informado é muito curto para continuar';

const EMAIL = 'Todos os emails informados devem ser válidos para continuar';

const CEP = 'O CEP deve seguir o formato: NNNNN-NNN';

const CPF = 'O CPF deve seguir o formato: NNN.NNN.NNN-NN';

const POSITION = 'O cargo informado não é um cargo válido';

const PHONE = 'O celular deve seguir o formato: (NN) NNNNN-NNNN';

const STATE = 'O estado deve ser informado utilizando uma sigla de duas letras';

const CITY = 'A cidade informada não pode possuir menos de três letras';

const NEIGHBORHOOD = 'O bairro informado é muito curto para continuar';

const STREET = 'A rua informada não pode possuir menos de três letras';

const NUMBER = 'O número deve possuir apenas caracteres numéricos';

const ADDRESSDETAILS = 'O complemento não pode possuir menos de três letras';

const BANK = 'O banco deve ser um número de três dígitos';

const BRANCH = 'A agência deve ser um número de quatro dígitos';

const ACCOUNT = 'A conta deve possuir apenas caracteres numéricos';

export const msg_errors = {
    REQUIRED,
    PHOTO,
    FULLNAME,
    EMAIL,
    CEP,
    CPF,
    POSITION,
    PHONE,
    STATE,
    CITY,
    NEIGHBORHOOD,
    STREET,
    NUMBER,
    ADDRESSDETAILS,
    BANK,
    BRANCH,
    ACCOUNT
};